...
namespace ORCA{
	...
	//TODO Da modificare per ritornarmi i valori
	void Agent::update()
	{
		newVelocityToPublish.linear.x = newVelocity_[0];
		newVelocityToPublish.linear.y = newVelocity_[1];
		newVelocityToPublish.linear.z = newVelocity_[2];
	}
	...
	void Agent::position_cb(const geometry_msgs::PoseStamped::ConstPtr& msg){ 
 	...
 	void Agent::vel_cb(const geometry_msgs::TwistStamped::ConstPtr& msg){
    	...
    	//velocity_[0] = (float)current_velocity.twist.linear.x;
    	//velocity_[1] = (float)current_velocity.twist.linear.y;
    	//velocity_[2] = (float)current_velocity.twist.linear.z;
	}
